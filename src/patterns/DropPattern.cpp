...
// 1D blob (soft)
inline void drawBlob1D(CRGB *leds, int count, int center, uint8_t radius,
                       const CRGB &color) {
  ...
  for (int i = start; i <= end; ++i) {
    ...
    uint8_t w = (radius == 0)
                    ? 255
                    : (uint8_t)((255 * (radius - d)) / (radius)); // 0..255
    w = scale8(w, w);                                             // soften
...
// 2D blob (soft)
inline void drawBlob2D(uint8_t cx, uint8_t cy, uint8_t radius,
                       const CRGB &color) {
...
inline CRGB randomPaletteColor() {
  return ColorFromPalette(currentPalette, random8(), g_paletteBrightness,
                          LINEARBLEND);
}
...
inline void spawnBarDrop(CRGB *bar, int count) {
  ...
  if (scaled < (int)g_minR1D)
    scaled = g_minR1D; // don’t get smaller than base min
  if (scaled > (int)g_barRadiusCap)
    scaled = g_barRadiusCap;
...
// -------------------- Public API --------------------
void dropPatternSetSpawnRates(float barsPerSec, float stripsPerSec,
                              float matrixPerSec) {
...
void dropPatternSetSizeRanges(uint8_t minRadius1D, uint8_t maxRadius1D,
  ...
  if (maxRadius1D < minRadius1D)
    maxRadius1D = minRadius1D;
  if (maxRadius2D < minRadius2D)
    maxRadius2D = minRadius2D;
...
void dropPatternSetPaletteBrightness(uint8_t v) { g_paletteBrightness = v; }
...
void renderDropPattern() {
  ...
  // Spawn whole events
  while (g_accBars >= 1.0f) {
    switch (random8(4)) {
    case 0:
      spawnBarDrop(bar_1, NUM_LEDS_BAR);
      break;
    case 1:
      spawnBarDrop(bar_2, NUM_LEDS_BAR);
      break;
    case 2:
      spawnBarDrop(bar_3, NUM_LEDS_BAR);
      break;
    default:
      spawnBarDrop(bar_4, NUM_LEDS_BAR);
      break;
    }
    g_accBars -= 1.0f;
  }
  while (g_accStrips >= 1.0f) {
    if (random8() & 1)
      spawnStripDrop(strip_1, NUM_LEDS_STRIP);
    else
      spawnStripDrop(strip_2, NUM_LEDS_STRIP);
    g_accStrips -= 1.0f;
  }
  while (g_accMatrix >= 1.0f) {
    spawnMatrixDrop();
    g_accMatrix -= 1.0f;
  }
